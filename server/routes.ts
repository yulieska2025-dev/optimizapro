...
app.use("/api", ClerkExpressWithAuth());
  ...
  app.post("/api/analyze", upload.single("file"), async (req: any, res) => {
    ...
    if (process.env.CLERK_SECRET_KEY) {
      ...
      // Registrar IP si es nuevo usuario
    ...
    try {
      ...
      if (isImage) {
        ...
        try {
          ...
          // --- BLOQUE DE SEGURIDAD: LECTURA DEL PROMPT SAGRADO ---
          // Se lee siempre desde el archivo para asegurar inmunidad ante cambios en el código.
          try {
            basePrompt = fs.readFileSync(path.join(process.cwd(), "prompt_base.txt"), "utf-8");
...
;
      ...
      // Incrementar contador solo para free y pro (ilimitado no lleva conteo)
  ...
  app.post("/api/stripe-webhook", async (req: any, res) => {
    // Logs ultradetallados al inicio
    ...
    if (rawBody) {
      ...
      console.log("🔔 Body (primeros 500 chars):", bodyStr.substring(0, 500));
      console.log("🔔 Body length total:", rawBody.length);
    } else {
      console.log("🔔 Body: VACÍO (undefined)");
    }
    
    const signature = req.headers["stripe-signature"] as string;
    console.log("🔔 Stripe-Signature header:", signature ? `${signature.substring(0, 30)}...` : "FALTA");
    
    if (!signature) {
      console.error("❌ Error: Falta stripe-signature header");
      return res.status(400).json({ error: "Falta stripe-signature" });
    }

    if (!rawBody) {
      console.error("❌ Error: No hay rawBody");
      return res.status(400).json({ error: "Sin body raw" });
    }

    try {
      console.log("🔔 Procesando webhook con handleStripeWebhook...");
  ...
  // DESHABILITADO: Endpoint de OpenAI Billing API está temporalmente fuera de servicio
  // Reactivar cuando se encuentre endpoint válido
